...
- (void)loadView
{
    ...
    _thumbsView.autoresizesSubviews				= NO;
    _thumbsView.alpha                           = .75;
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:@"全部" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
...
- (void)resizeThumbView
{
    int navbarHeight = 0, totalHeight = 0;
    if (self.navigationController.navigationBar.translucent) {
        totalHeight = navbarHeight = self.navigationController.navigationBar.frame.size.height;
    }
    if (self.navigationController.toolbar.translucent) {
        totalHeight += self.navigationController.toolbar.frame.size.height;
    }
	_thumbsView.frame = CGRectMake( 0, navbarHeight, _container.frame.size.width, _container.frame.size.height-totalHeight );
}
	...
	if([_photoSource numberOfPhotosForPhotoGallery:self] > 0 )
	{
		if([_photoSource respondsToSelector:@selector(photoGallery:captionForPhotoAtIndex:)])
		{
			...
			if([caption length] > 0 )
			{
				...
				NSInteger height = ( textSize.height + kCaptionPadding );
...
{
    static int kCaptionHeight = 27;
    int kToolBarHeight = [self navigationController].toolbar.frame.size.height;
	_captionContainer.frame = CGRectMake( 0, _scroller.frame.size.height - kCaptionHeight - kToolBarHeight, _scroller.frame.size.width, kCaptionHeight );
}
...
- (void)toggleThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    //[self setBarTranslucent: !_isThumbViewShowing];
}
